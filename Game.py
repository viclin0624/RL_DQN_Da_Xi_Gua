...
class GameBoard(object):
    def __init__(self, create_time, gravity):
        self.RES = self.WIDTH, self.HEIGHT = 568, 875#400 800
        self.FPS = 80
        self.balls = []
        self.fruits = []
        ...
        self.init_y = int(0.1 * self.HEIGHT)
    ...
    def setup_collision_handler(self):
        def post_solve_bird_line(arbiter, space, data):
            if not self.lock:
                ...
                if arbiter.shapes[0] in self.balls and arbiter.shapes[1] in self.balls: #防止出现多次合并
                    if y1 > y2:
                        x, y = x1, y1
                    else:
                        x, y = x2, y2
    ...
    def create_ball(self, space, x, y, m=1, r=7, i=1):
        ...
        ball_shape.elasticity = 0.3#*(i/12) #一个与原有游戏更类似的弹性系数
    ...
    def check_fail(self):
        ...
        if len(self.balls):
            for i, ball in enumerate(self.balls[:-1]):
                if ball:
                    if int(ball.body.position[1]-ball.radius) < 1.5*self.init_y:
                        self.fail_count += 1
                        exist = True
                        break
        if exist:
